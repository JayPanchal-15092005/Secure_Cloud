...
export default function FileList({
  ...
  const openFileViewer = (file: FileType) => {
    ...
    if (type.startsWith("image/")) {
      const url = `${base}/tr:q-90,w-1600,h-1200,fo-auto/${path}`;
      window.open(url, "_blank");
      return;
    }

    // VIDEO: open the direct fileUrl (most browsers will play it inline)
    if (type.startsWith("video/")) {
      window.open(fileUrl, "_blank");
      return;
    }

    // PDF: open the PDF URL, browsers render inline
    if (type === "application/pdf") {
      window.open(fileUrl, "_blank");
      return;
    }

    // FALLBACK: for everything else, just open in new tab
    window.open(fileUrl, "_blank");
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {filteredFiles.length === 0 ? (
        ...
        <Card className="border border-default-200 bg-default-50 overflow-hidden">
